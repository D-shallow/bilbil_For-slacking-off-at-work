...
function getVidWithP(url) {
    // 放宽限制：无论在什么路径下，只要包含 av/BV 就能瞬间识别，兼容大小写
    const m = /(av\d+(?:\/?\?p=\d+)?)/i.exec(url) || /(BV\w+(?:\/?\?p=\d+)?)/i.exec(url);
    return m ? m[1].replace(/^bv/i, 'BV') : null;
}

function getVid(url) {
    const m = /(av\d+)/i.exec(url) || /(BV\w+)/i.exec(url);
    return m ? m[1].replace(/^bv/i, 'BV') : null;
}
